<template>
  <div class="container" style="background-color: #4e73df">
    <div class="row pt-3">
      <div class="col-md-6">
        <h1 style="font-size: 24px; font-weight:bold text-white">ALICORN</h1>
      </div>
      <div class="col-md-6">
        <a href="https://t.me/MrAlicorn" class="float-end text-white">Contact Us</a>
      </div>
    </div>
    <div class="row mt-5">
      <div class="col-md-6">
        <h1 style="font-size: 2rem;" class="text-white">ALICORN made you trade easier</h1>
        <p class="text-white">
          Alicorn is a 24-hour live signal exclusively for Market Crypto IDX. 
          <strong style="font-size: 1.2rem;" class="text-white">WHY ALICORN ?</strong>
          <ul>
            <li>Easy to use</li>
            <li>24-hour running</li>
            <li>NO DELAY</li>
            <li>OP MANUAL</li>
            <li>FREE CONSULTATION</li>
            <li>FAST RESPOND</li>
            <li>WINRATE INDICATOR &gt; 90%</li>
          </ul>
        </p>
      </div>
      <div class="col-md-6">
        <div class="kotak">
          <img src="/img/logo.jpg" alt="ALc" width="300">
        </div>
      </div>
    </div>
    <div class="row mt-5 pb-5">
      <div class="col-md-6">
        <a href="/login">
          <button class="btn btn-warning float-end">Login</button>
        </a>
      </div>
      <div class="col-md-6">
        <a href="/register">
          <button class="btn btn-success">Register</button>
        </a>
      </div>
    </div>
  </div>
</template>

<style scoped>
h1,
p,
strong,
li {
  color: white;
}

.container {
  border-radius: 0px 0px 60px 0px;
}
</style>
